...
void Audio::destroy_decoder() {
    if (m_decoder && m_decoder->isValid()) {
        info1(*this, evt_info, "{}Decoder has been destroyed", m_decoder->whoIsIt());
        m_decoder->reset();
        m_decoder.reset();
    }
}
...
void Audio::initInBuff() {
    if (!InBuff.isInitialized()) {
        ...
        if (size > 0) { info1(*this, evt_info, "inputBufferSize: {} bytes", size - 1); }
    }
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
bool Audio::openai_speech(const String& api_key, const String& model, const String& input, const String& instructions, const String& voice, const String& response_format, const String& speed) {
    ...
    uint32_t t = millis();
    info1(*this, evt_info, "Connect to: \"{}\"", host.get());
    res = m_client->connect(host.get(), port, m_timeout_ms_ssl);
    if (res) {
        uint32_t dt = millis() - t;
        m_lastHost.assign(host.get());
        m_currentHost.clone_from(host);
        info1(*this, evt_info, "{} has been established in {} ms", m_f_ssl ? "SSL" : "Connection", (long unsigned int)dt);
        m_f_running = true;
    }
...
bool Audio::connecttohost(const char* host, const char* user, const char* pwd) { // user and pwd for authentification only, can be empty
    ...
    info1(*this, evt_info, "connect to: \"{}\" on port {} path \"/{}\"", hwoe.get(), port, path.get());
    ...
    if (res) {
        ...
        info1(*this, evt_lasthost, "{}", m_lastHost.c_get());
        ...
        m_f_running = false;
        info1(*this, evt_name, "");
        info1(*this, evt_streamtitle, "");
        info1(*this, evt_icydescription, "");
        info1(*this, evt_icyurl, "");
    }
    xSemaphoreGiveRecursive(mutex_playAudioData);
    return res;
}
...
bool Audio::httpPrint(const char* host) {
    ...
    info1(*this, evt_info, "next URL: \"{}\"", c_host.get());
    ...
    if (!m_client->connected()) {
        ...
        if (f_equal) info1(*this, evt_info, "The host has disconnected, reconnecting");
...
bool Audio::connecttoFS(fs::FS& fs, const char* path, int32_t fileStartTime) {
    ...
    info1(*this, evt_info, "Reading file: \"{}\"", c_path.get());
...
bool Audio::connecttospeech(const char* speech, const char* lang) {
    ...
    m_client = static_cast<NetworkClient*>(&client);
    info1(*this, evt_info, "connect to \"{}\"", host);
...
void Audio::showID3Tag(const char* tag, const char* value) {
    ps_ptr<char> id3tag;
    // V2.2
    if (!strcmp(tag, "CNT")) id3tag.appendf("Play counter: {}", value, "id3tag");
    if (!strcmp(tag, "COM")) id3tag.appendf("Comments: {}", value, "id3tag");
    if (!strcmp(tag, "CRA")) id3tag.appendf("Audio encryption: {}", value, "id3tag");
    if (!strcmp(tag, "CRM")) id3tag.appendf("Encrypted meta frame: {}", value, "id3tag");
    if (!strcmp(tag, "ETC")) id3tag.appendf("Event timing codes: {}", value, "id3tag");
    if (!strcmp(tag, "EQU")) id3tag.appendf("Equalization: {}", value, "id3tag");
    if (!strcmp(tag, "IPL")) id3tag.appendf("Involved people list: {}", value, "id3tag");
    if (!strcmp(tag, "PIC")) id3tag.appendf("Attached picture: {}", value, "id3tag");
    if (!strcmp(tag, "SLT")) id3tag.appendf("Synchronized lyric/text: {}", value, "id3tag");
    if (!strcmp(tag, "TAL")) id3tag.appendf("Album/Movie/Show title: {}", value, "id3tag");
    if (!strcmp(tag, "TBP")) id3tag.appendf("BPM (Beats Per Minute): {}", value, "id3tag");
    if (!strcmp(tag, "TCM")) id3tag.appendf("Composer: {}", value, "id3tag");
    if (!strcmp(tag, "TCO")) id3tag.appendf("Content type: {}", value, "id3tag");
    if (!strcmp(tag, "TCR")) id3tag.appendf("Copyright message: {}", value, "id3tag");
    if (!strcmp(tag, "TDA")) id3tag.appendf("Date: {}", value, "id3tag");
    if (!strcmp(tag, "TDY")) id3tag.appendf("Playlist delay: {}", value, "id3tag");
    if (!strcmp(tag, "TEN")) id3tag.appendf("Encoded by: {}", value, "id3tag");
    if (!strcmp(tag, "TFT")) id3tag.appendf("File type: {}", value, "id3tag");
    if (!strcmp(tag, "TIM")) id3tag.appendf("Time: {}", value, "id3tag");
    if (!strcmp(tag, "TKE")) id3tag.appendf("Initial key: {}", value, "id3tag");
    if (!strcmp(tag, "TLA")) id3tag.appendf("Language(s): {}", value, "id3tag");
    if (!strcmp(tag, "TLE")) id3tag.appendf("Length: {}", value, "id3tag");
    if (!strcmp(tag, "TMT")) id3tag.appendf("Media type: {}", value, "id3tag");
    if (!strcmp(tag, "TOA")) id3tag.appendf("Original artist(s)/performer(s): {}", value, "id3tag");
    if (!strcmp(tag, "TOF")) id3tag.appendf("Original filename: {}", value, "id3tag");
    if (!strcmp(tag, "TOL")) id3tag.appendf("Original Lyricist(s)/text writer(s): {}", value, "id3tag");
    if (!strcmp(tag, "TOR")) id3tag.appendf("Original release year: {}", value, "id3tag");
    if (!strcmp(tag, "TOT")) id3tag.appendf("Original album/Movie/Show title: {}", value, "id3tag");
    if (!strcmp(tag, "TP1")) id3tag.appendf("Lead artist(s)/Lead performer(s)/Soloist(s)/Performing group: {}", value, "id3tag");
    if (!strcmp(tag, "TP2")) id3tag.appendf("Band/Orchestra/Accompaniment: {}", value, "id3tag");
    if (!strcmp(tag, "TP3")) id3tag.appendf("Conductor/Performer refinement: {}", value, "id3tag");
    if (!strcmp(tag, "TP4")) id3tag.appendf("Interpreted, remixed, or otherwise modified by: {}", value, "id3tag");
    if (!strcmp(tag, "TPA")) id3tag.appendf("Part of a set: {}", value, "id3tag");
    if (!strcmp(tag, "TPB")) id3tag.appendf("Publisher: {}", value, "id3tag");
    if (!strcmp(tag, "TRC")) id3tag.appendf("ISRC (International Standard Recording Code): {}", value, "id3tag");
    if (!strcmp(tag, "TRD")) id3tag.appendf("Recording dates: {}", value, "id3tag");
    if (!strcmp(tag, "TRK")) id3tag.appendf("Track number/Position in set: {}", value, "id3tag");
    if (!strcmp(tag, "TSI")) id3tag.appendf("Size: {}", value, "id3tag");
    if (!strcmp(tag, "TSS")) id3tag.appendf("Software/hardware and settings used for encoding: {}", value, "id3tag");
    if (!strcmp(tag, "TT1")) id3tag.appendf("Content group description: {}", value, "id3tag");
    if (!strcmp(tag, "TT2")) id3tag.appendf("Title/Songname/Content description: {}", value, "id3tag");
    if (!strcmp(tag, "TT3")) id3tag.appendf("Subtitle/Description refinement: {}", value, "id3tag");
    if (!strcmp(tag, "TXT")) id3tag.appendf("Lyricist/text writer: {}", value, "id3tag");
    if (!strcmp(tag, "TXX")) id3tag.appendf("User defined text information frame: {}", value, "id3tag");
    if (!strcmp(tag, "TYE")) id3tag.appendf("Year: {}", value, "id3tag");
    if (!strcmp(tag, "UFI")) id3tag.appendf("Unique file identifier: {}", value, "id3tag");
    if (!strcmp(tag, "ULT")) id3tag.appendf("Unsychronized lyric/text transcription: {}", value, "id3tag");
    if (!strcmp(tag, "WAF")) id3tag.appendf("Official audio file webpage: {}", value, "id3tag");
    if (!strcmp(tag, "WAR")) id3tag.appendf("Official artist/performer webpage: {}", value, "id3tag");
    if (!strcmp(tag, "WAS")) id3tag.appendf("Official audio source webpage: {}", value, "id3tag");
    if (!strcmp(tag, "WCM")) id3tag.appendf("Commercial information: {}", value, "id3tag");
    if (!strcmp(tag, "WCP")) id3tag.appendf("Copyright/Legal information: {}", value, "id3tag");
    if (!strcmp(tag, "WPB")) id3tag.appendf("Publishers official webpage: {}", value, "id3tag");
    if (!strcmp(tag, "WXX")) id3tag.appendf("User defined URL link frame: {}", value, "id3tag");

    // V2.3 V2.4 tags
    if (!strcmp(tag, "COMM")) id3tag.appendf("Comment: {}", value, "id3tag");
    if (!strcmp(tag, "OWNE")) id3tag.appendf("Ownership: {}", value, "id3tag");
    if (!strcmp(tag, "PRIV")) id3tag.appendf("Private: {}", value, "id3tag");
    if (!strcmp(tag, "SYLT")) id3tag.appendf("SynLyrics: {}", value, "id3tag");
    if (!strcmp(tag, "TALB")) id3tag.appendf("Album: {}", value, "id3tag");
    if (!strcmp(tag, "TBPM")) id3tag.appendf("BeatsPerMinute: {}", value, "id3tag");
    if (!strcmp(tag, "TCAT")) id3tag.appendf("PodcastCategory: {}", value, "id3tag");
    if (!strcmp(tag, "TCMP")) id3tag.appendf("Compilation: {}", value, "id3tag");
    if (!strcmp(tag, "TCOM")) id3tag.appendf("Composer: {}", value, "id3tag");
    if (!strcmp(tag, "TCON")) id3tag.appendf("ContentType: {}", value, "id3tag");
    if (!strcmp(tag, "TCOP")) id3tag.appendf("Copyright: {}", value, "id3tag");
    if (!strcmp(tag, "TDAT")) id3tag.appendf("Date: {}", value, "id3tag");
    if (!strcmp(tag, "TDES")) id3tag.appendf("PodcastDescription: {}", value, "id3tag");
    if (!strcmp(tag, "TDOR")) id3tag.appendf("Original Release Year: {}", value, "id3tag");
    if (!strcmp(tag, "TDRC")) id3tag.appendf("Publication date: {}", value, "id3tag");
    if (!strcmp(tag, "TDRL")) id3tag.appendf("ReleaseTime: {}", value, "id3tag");
    if (!strcmp(tag, "TENC")) id3tag.appendf("Encoded by: {}", value, "id3tag");
    if (!strcmp(tag, "TEXT")) {} // id3tag.appendf("Lyricist: {}", value, "id3tag");
    if (!strcmp(tag, "TGID")) id3tag.appendf("PodcastID: {}", value, "id3tag");
    if (!strcmp(tag, "TIME")) id3tag.appendf("Time: {}", value, "id3tag");
    if (!strcmp(tag, "TIT1")) id3tag.appendf("Grouping: {}", value, "id3tag");
    if (!strcmp(tag, "TIT2")) id3tag.appendf("Title: {}", value, "id3tag");
    if (!strcmp(tag, "TIT3")) id3tag.appendf("Subtitle: {}", value, "id3tag");
    if (!strcmp(tag, "TLAN")) id3tag.appendf("Language: {}", value, "id3tag");
    if (!strcmp(tag, "TLEN")) id3tag.appendf("Length (ms): {}", value, "id3tag");
    if (!strcmp(tag, "TMED")) id3tag.appendf("Media: {}", value, "id3tag");
    if (!strcmp(tag, "TOAL")) id3tag.appendf("OriginalAlbum: {}", value, "id3tag");
    if (!strcmp(tag, "TOPE")) id3tag.appendf("OriginalArtist: {}", value, "id3tag");
    if (!strcmp(tag, "TOLY")) id3tag.appendf("OriginalLyricist: {}", value, "id3tag");
    if (!strcmp(tag, "TORY")) id3tag.appendf("OriginalReleaseYear: {}", value, "id3tag");
    if (!strcmp(tag, "TPE1")) id3tag.appendf("Artist: {}", value, "id3tag");
    if (!strcmp(tag, "TPE2")) id3tag.appendf("Band: {}", value, "id3tag");
    if (!strcmp(tag, "TPE3")) id3tag.appendf("Conductor: {}", value, "id3tag");
    if (!strcmp(tag, "TPE4")) id3tag.appendf("InterpretedBy: {}", value, "id3tag");
    if (!strcmp(tag, "TPOS")) id3tag.appendf("PartOfSet: {}", value, "id3tag");
    if (!strcmp(tag, "TPUB")) id3tag.appendf("Publisher: {}", value, "id3tag");
    if (!strcmp(tag, "TRCK")) id3tag.appendf("Track: {}", value, "id3tag");
    if (!strcmp(tag, "TRSN")) id3tag.appendf("InternetRadioStationName: {}", value, "id3tag");
    if (!strcmp(tag, "TSSE")) id3tag.appendf("SettingsForEncoding: {}", value, "id3tag");
    if (!strcmp(tag, "TRDA")) id3tag.appendf("RecordingDates: {}", value, "id3tag");
    if (!strcmp(tag, "TSO2")) id3tag.appendf("AlbumArtistSortOrder: {}", value, "id3tag");
    if (!strcmp(tag, "TSOA")) id3tag.appendf("AlbumSortOrder: {}", value, "id3tag");
    if (!strcmp(tag, "TSOP")) id3tag.appendf("PerformerSortOrder: {}", value, "id3tag");
    if (!strcmp(tag, "TSOC")) id3tag.appendf("ComposerSortOrder: {}", value, "id3tag");
    if (!strcmp(tag, "TSOT")) id3tag.appendf("TitleSortOrder: {}", value, "id3tag");
    if (!strcmp(tag, "TXXX")) id3tag.appendf("UserDefinedText: {}", value, "id3tag");
    if (!strcmp(tag, "TYER")) id3tag.appendf("Year: {}", value, "id3tag");
    if (!strcmp(tag, "USER")) id3tag.appendf("TermsOfUse: {}", value, "id3tag");
    if (!strcmp(tag, "USLT")) id3tag.appendf("Lyrics: {}", value, "id3tag");
    if (!strcmp(tag, "WCOM")) id3tag.appendf("Commercial URL: {}", value, "id3tag");
    if (!strcmp(tag, "WFED")) id3tag.appendf("Podcast URL: {}", value, "id3tag");
    if (!strcmp(tag, "WOAF")) id3tag.appendf("File URL: {}", value, "id3tag");
    if (!strcmp(tag, "WOAR")) id3tag.appendf("OfficialArtistWebpage: {}", value, "id3tag");
    if (!strcmp(tag, "WOAS")) id3tag.appendf("Source URL: {}", value, "id3tag");
    if (!strcmp(tag, "WORS")) id3tag.appendf("InternetRadioStationURL: {}", value, "id3tag");
    if (!strcmp(tag, "WXXX")) id3tag.appendf("User defined URL link frame: {}", value, "id3tag");
    if (!strcmp(tag, "XDOR")) id3tag.appendf("OriginalReleaseTime: {}", value, "id3tag");
    ...
    if (id3tag.strlen()) { info1(*this, evt_id3data, "{}", id3tag.get()); }
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
int Audio::read_WAV_Header(uint8_t* data, size_t len) {
    ...
    if (m_controlCounter == 5) {
        ...
        info1(*this, evt_info, "FormatCode: {}", fc);
        // info1(*this, evt_info, "Channel: {}", nic);
        // info1(*this, evt_info, "SampleRate (Hz): {}", sr);
        info1(*this, evt_info, "DataRate: {}", (long unsigned int)dr);
        info1(*this, evt_info, "DataBlockSize: {}", dbs);
        info1(*this, evt_info, "BitsPerSample: {}", bps);

        if ((bps != 8) && (bps != 16) && (bps != 24) && (bps != 32)) {
            info1(*this, evt_info, "BitsPerSample is {},  must be 8, 16, 24 or 32", bps);
            stopSong();
            return -1;
        }
        if ((ch != 1) && (ch != 2)) {
            info1(*this, evt_info, "num channels is {},  must be 1 or 2", ch);
            stopSong();
            return -1;
        }
        if (fc != 1) {
    ...
    if (m_controlCounter == 8) {
        ...
        info1(*this, evt_info, "Duration (s): {}", m_audioFileDuration);
        info1(*this, evt_bitrate, "{}", m_nominal_bitrate);
        info1(*this, evt_info, "Bitrate (b/s): {}", m_nominal_bitrate);
        return 4;
    }
    m_controlCounter = 100; // header succesfully read
    m_audioDataStart = m_rwh.headerSize;
    info1(*this, evt_info, "Audio-Data-Start: {}", m_audioDataStart);
    info1(*this, evt_info, "Audio-Length: {}", m_audioDataSize);
    return 0;
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
int Audio::read_FLAC_Header(uint8_t* data, size_t len) {
    ...
    if (m_controlCounter == FLAC_MBH) { /* METADATA_BLOCK_HEADER */
        ...
        if (m_rflh.duration) {
            ...
            info1(*this, evt_info, "Duration (s): {}", m_rflh.duration);
            info1(*this, evt_bitrate, "{}", m_nominal_bitrate);
            info1(*this, evt_info, "Bitrate (b/s): {}", m_nominal_bitrate);
        }
        m_rflh.retvalue = 0;
    ...
    if (m_controlCounter == FLAC_SINFO) { /* Stream info block */
        ...
        info1(*this, evt_info, "FLAC maxBlockSize: {}", m_rflh.maxBlockSize);
        vTaskDelay(2);
        m_rflh.maxFrameSize = bigEndian(data + 10, 3);
        if (m_rflh.maxFrameSize) {
            info1(*this, evt_info, "FLAC maxFrameSize: {}", m_rflh.maxFrameSize);
        } else {
            info1(*this, evt_info, "FLAC maxFrameSize: N/A");
        }
        ...
        info1(*this, evt_info, "FLAC sampleRate (Hz): {}", (long unsigned int)m_rflh.sampleRate);
        vTaskDelay(2);
        m_rflh.numChannels = ((nextval & 0x06) >> 1) + 1;
        info1(*this, evt_info, "FLAC numChannels: {}", m_rflh.numChannels);
        vTaskDelay(2);
        ...
        info1(*this, evt_info, "FLAC bitsPerSample: {}", m_rflh.bitsPerSample);
        m_rflh.totalSamplesInStream = bigEndian(data + 17, 4);
        if (m_rflh.totalSamplesInStream) {
            info1(*this, evt_info, "total samples in stream: {}", (long unsigned int)m_rflh.totalSamplesInStream);
        } else {
            info1(*this, evt_info, "total samples in stream: N/A");
        }
    ...
    if (m_controlCounter == FLAC_VORBIS) { /* VORBIS COMMENT */ // field names
        ...
        info1(*this, evt_id3data, "{}", vendorString.get());
        ...
        for (int i = 0; i < commentListLength; i++) {
            ...
            if (commentLength) { // guard
                ...
                if (commentString.starts_with("LYRICS="))
                    ...
                    info1(*this, evt_id3data, "{}", commentString.get());
            }
            data += commentLength;
            idx += commentLength;
        ...
        if (lyricsBuffer.valid()) {
            ...
            info1(*this, evt_info, "audiofile contains synchronized lyrics");
            // for(int i = 0; i < m_syltLines.size(); i++){
            //     info1(*this, evt_lyrics, "%07i ms,   {}", m_syltTimeStamp[i], m_syltLines[i].c_get());
            // }
        }
        m_controlCounter = FLAC_MBH;
...
int Audio::read_ID3_Header(uint8_t* data, size_t len) {
    ...
    if (m_controlCounter == MP3_ID3HEADER) {
        ...
        int retval = 0;
        if (!m_f_m3u8data) info1(*this, evt_info, "File-Size: {}", m_audioFileSize);

        m_ID3Hdr.remainingHeaderBytes = 0;
        m_ID3Hdr.ehsz = 0;
        if (specialIndexOf(data, "ID3", 4) != 0) { // ID3 not found
            if (!m_f_m3u8data) { info1(*this, evt_info, "file has no ID3 tag, skip metadata"); }
            m_audioDataSize = m_audioFileSize;
            // if(!m_f_m3u8data) info1(*this, evt_info, "Audio-Length: {}", m_audioDataSize);
        ...
        if (!m_f_m3u8data) info1(*this, evt_info, "ID3 framesSize: {}", m_ID3Hdr.id3Size);
        if (!m_f_m3u8data) info1(*this, evt_info, "ID3 version: 2.{}", m_ID3Hdr.ID3version);
        ...
        if (m_f_exthdr) {
            ...
            info1(*this, evt_info, "ID3 extended header, size: {}", m_ID3Hdr.ehsz);
            m_ID3Hdr.ehsz -= 4;
    ...
    if (m_controlCounter == MP3_TAG) { // Read the value
        ...
        if (startsWith(m_ID3Hdr.tag, "COMM")) { // language code
            ...
            if (cd_len > 2) info1(*this, evt_info, "Comment: text encoding; {}, language {}, content_descriptor: {}", encodingTab[textEncodingByte], m_ID3Hdr.lang, content_descriptor.c_get());
    ...
    if (m_controlCounter == MP3_SYLT) { // SYLT
        ...
        if (m_dataMode == AUDIO_LOCALFILE || (m_streamType == ST_WEBFILE)) {
            ...
            info1(*this, evt_info, "Lyrics: text_encoding: {}, language: {}, size {}", encodingTab[m_ID3Hdr.SYLT.text_encoding], m_ID3Hdr.SYLT.lang, m_ID3Hdr.SYLT.size);
            ...
            if (cd_len > 2) info1(*this, evt_info, "Lyrics: content_descriptor: {}", content_descriptor.c_get());
            ...
            info1(*this, evt_info, "audiofile contains synchronized lyrics");
            // for(int i = 0; i < m_syltLines.size(); i++){
            //     info1(*this, evt_lyrics, "{:07} ms,   {}", m_syltTimeStamp[i], m_syltLines[i].c_get());
            // }
        }
        return 0;
    }
    ...
    if (m_controlCounter == MP3_ID3V22) { // frames in V2.2, 3bytes identifier, 3bytes size descriptor
        ...
        if (tag.starts_with("PIC")) { // image embedded in header
            ...
            if (m_dataMode == AUDIO_LOCALFILE) {
                ...
                memcpy(m_ID3Hdr.SYLT.lang, syltBuff.get() + 1, 3);
                m_ID3Hdr.SYLT.lang[3] = '\0';
                info1(*this, evt_info, "Lyrics: text_encoding: {}, language: {}, size {}",
                ...
                if (len > 2) info1(*this, evt_info, "Lyrics: content_descriptor: {}", content_descriptor.c_get());
                idx += len;
    ...
    if (m_controlCounter == MP3_XING) { //  exist another ID3tag?
        ...
        if ((*(data + 0) == 'I') && (*(data + 1) == 'D') && (*(data + 2) == '3')) {
            ...
            m_audioDataSize = m_audioFileSize - m_audioDataStart;
            if (!m_f_m3u8data) info1(*this, evt_info, "Audio-Data-Start: {}", m_audioDataStart);
            if (!m_f_m3u8data) info1(*this, evt_info, "Audio-Length: {}", m_audioDataSize);
            ...
            if (xingPos > 0 && layerIndex == 1) { // layer III only
                ...
                info1(*this, evt_info, "Duration (s): {}", duration);
                m_audioFileDuration = duration;
                uint32_t bitrate = bytes * 8 / duration;
                info1(*this, evt_info, "Bitrate (b/s): {}", bitrate);
...
int Audio::read_M4A_Header(uint8_t* data, size_t len) {
    ...
    if (m_controlCounter == M4A_CHK) { /* check  Tag */
        ...
        if (atom_name.equals("mdat")) {
            ...
            if (!m_m4aHdr.progressive) {
                ...
                info1(*this, evt_info, "Audiofile is non progressive");
    ...
    if (m_controlCounter == M4A_CHPL) {
        ...
        info1(*this, evt_info, "Number of chapters: {}", nr_of_chapters);
        info1(*this, evt_info, "Chapter name: {}", title_name.c_get());
    ...
    if (m_controlCounter == M4A_ESDS) {                           // Elementary Stream Descriptor
        ...
        if (dec_config_descriptor_offset > 0) {                                                                     // decoderConfigDescriptor found
            ...
            // info1(*this, evt_info, "maxBitrate {}, nominalBitrate {}", m_m4aHdr.maxBitrate, m_m4aHdr.nomBitrate);
        }
        ...
        if (dec_specific_offset >= 0) {                                                           // decoderSpecificInfo found
            ...
            AUDIO_LOG_DEBUG("DecoderSpecificInfo found at offset {}, length: %u", dec_specific_offset, dec_specific_length);
            ...
            info1(*this, evt_info, "AAC Profile: {}", profile[m_m4aHdr.aac_profile]);
            info1(*this, evt_info, "AAC Channels; {}", m_m4aHdr.channel_count);
            info1(*this, evt_info, "AAC Sample Rate: {}", m_m4aHdr.sample_rate);
            info1(*this, evt_info, "AAC Bits Per Sample: {}", m_m4aHdr.sample_size);
            ...
            if (strncmp(ssan, "data", 4) == 0) {
                for (int i = 0; i < tags_count; i++) {
                    if (memcmp(san, tags[i].tag, 4) == 0) {
                        ...
                        if (id3tag.valid()) { info1(*this, evt_id3data, "{}", id3tag.get()); }
...
uint32_t Audio::stopSong() {
    ...
    xSemaphoreTake(mutex_audioTaskIsDecoding, 0.3 * configTICK_RATE_HZ); // wait for audioTask is ready
    {
        if (m_f_running) {
            m_f_running = false;
            if (m_client->connected()) {
                if (m_streamType == ST_WEBSTREAM) { info1(*this, evt_info, "Closing web stream \"{}\"", m_lastHost.c_get()); }
                if (m_streamType == ST_WEBFILE) { info1(*this, evt_info, "Closing web file \"{}\"", m_lastHost.c_get()); }
                m_client->stop();
            }
            if (m_audiofile) {
                info1(*this, evt_info, "Closing audio file \"{}\"", m_audiofile.name());
                m_audiofile.close();
            }
        }
        destroy_decoder();
    ...
    for (int i = 0; i < lines; i++) {
        ...
        if (m_playlistContent[i].contains("#EXTINF:")) { // Info?
            ...
            if (pos > 0) {
                ...
                info1(*this, evt_id3data, "{}", m_playlistContent[i].get() + pos + 1);
            }
            continue;
        }
        ...
        if (pos >= 0) {                                 // Does URL contain "http://"?
                                                        //    AUDIO_LOG_ERROR("{} pos=%i", m_playlistContent[i], pos);
...
const char* Audio::parsePlaylist_PLS() {
    ...
    for (int i = 0; i < lines; i++) {
        ...
        if (m_playlistContent[i].starts_with("Title1")) { // Title1=Antenne Tirol
            ...
            info1(*this, evt_name, "{}", plsStationName);
            continue;
        }
...
void Audio::processLocalFile() {
    ...
    if (!m_f_stream) {
        if (m_controlCounter != 100) {
            ...
            m_f_stream = true;
            info1(*this, evt_info, "stream ready");
        }
    }
    ...
    if (m_fileStartTime > 0 && m_nominal_bitrate) {
        if (getBitRate() > 0)
            ...
            info1(*this, evt_info, "can't set audio play time directly");
        m_fileStartTime = -1;
    }
    ...
    exit:
        ...
        if (afn.valid()) { info1(*this, evt_eof, "{}", afn.c_get()); }
        return;
    }
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————-
void Audio::processWebStream() {
    ...
    if (m_f_eof) {
        info1(*this, evt_eof, "{}", m_lastHost.c_get());
        stopSong();
    }
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
chunkFinished:
    ...
    if (true) {                                             // statement has no effect
        if (InBuff.bufferFilled() > 60000 && !m_f_stream) { // waiting for buffer filled
            ...
            info1(*this, evt_info, "stream ready");
            info1(*this, evt_info, "buffer filled in {} ms", filltime);
        }
    }
    ...
    while (true) { // read the header line for line
        ...
        if (rhl.starts_with_icase("HTTP/")) { // HTTP status error code
            ...
            if (sc > 310) { // e.g. HTTP/1.1 301 Moved Permanently
                info1(*this, evt_streamtitle, "{}", rhl.get());
                goto exit;
            }
                ...
                if (!m_currentHost.equals(c_host)) { // prevent a loop
                    ...
                    if (pos_slash > 9) {
                        if (!strncmp(c_host, m_currentHost.get(), pos_slash)) {
                            info1(*this, evt_info, "redirect to new extension at existing host \"{}\"", c_host);
                    ...
                    info1(*this, evt_info, "redirect to new host \"{}\"", c_host);
                    m_f_reset_m3u8Codec = false;
                    httpPrint(c_host);
                    return true;
                }
            ...
            if (idx >= 0) {
                ...
                info1(*this, evt_info, "Filename is {}", fn.get());
            }
            ...
            info1(*this, evt_info, "icy-genre: {}", rhl.get() + 10); // Ambient, Rock, etc
        }
            ...
            icyLogo.trim();
            if (icyLogo.strlen() > 0) {
                info1(*this, evt_info, "icy-logo: {}", icyLogo.get());
                info1(*this, evt_icylogo, "{}", icyLogo.get());
            }
        }
            ...
            icyName.trim();
            if (icyName.strlen() > 0) { info1(*this, evt_name, "{}", icyName.get()); }
        }
            ...
            // info1(*this, evt_info, "content-length: {}", (long unsigned int)m_audioFileSize);
        }
...
bool Audio::parseHttpRangeHeader() { // this is the response to a Range request
    ...
    while (true) { // outer while
        ...
        if (rhl.starts_with_icase("HTTP/")) { // HTTP status error code
            ...
            int sc = atoi(statusCode);
            if (sc > 310) { // e.g. HTTP/1.1 301 Moved Permanently
                info1(*this, evt_name, "{}", rhl.get());
                goto exit;
            }
        }
        if (rhl.starts_with_icase("Server:")) { info1(*this, evt_info, "{}", rhl.c_get()); }
        if (rhl.starts_with_icase("Content-Length:")) {
            //    info1(*this, evt_info, "{}", rhl.c_get());
        }
        if (rhl.starts_with_icase("Content-Range:")) { info1(*this, evt_info, "{}", rhl.c_get()); }
...
bool Audio::initializeDecoder() {
    ...
    info1(*this, evt_info, "{}Decoder has been initialized", m_decoder->whoIsIt());
    return true;
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
void Audio::showstreamtitle(char* st) {
    ...
    if (ml.index_of("StreamUrl=", 0) > 0) {
        ...
        if (idx1 >= 0 && idx2 > idx1) { // StreamURL found
            ...
            if (!sUrl.equals(m_streamURL.c_get())) {
                info1(*this, evt_info, "Stream URL: {}", sUrl.c_get()); // e.g. StreamUrl='http://myUrl.com'
                m_streamURL = sUrl;
            }
        }
    }
    ...
    if (ml.index_of("adw_ad=", 0) > 0) {
        ...
        if (idx1 >= 0) { // Advertisement found
            ...
            if (idx1 >= 0 && idx2 > idx1) {
                ...
                // info1(*this, evt_info, "Adveritsement: {}", sAdv.get());
                ...
                info1(*this, evt_info, "Advertisement: {}", sAdv.get() + pos);
            }
        }
        return;
    }
    ...
    info1(*this, evt_streamtitle, "{}", m_streamTitle.c_get());
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
void Audio::showCodecParams() {

    info1(*this, evt_info, "Channels: {}", getChannels());
    info1(*this, evt_info, "SampleRate (Hz): {}", getSampleRate());
    info1(*this, evt_info, "BitsPerSample: {}", getBitsPerSample());
    // if(getBitRate()) { info1(*this, evt_info, "BitRate (b/s): {}", getBitRate()); }
    // else { info1(*this, evt_info, "BitRate (b/s): N/A"); }

    if (m_codec == CODEC_AAC) {
        info1(*this, evt_info, "{}", m_decoder->arg2()); // AAC Format
        ...
        if (answ > 0 && answ < 4) {
            ...
            info1(*this, evt_info, "Spectral band replication: {}", sbr[answ]);
        }
    }
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
int Audio::findNextSync(uint8_t* data, size_t len) {
    ...
    if (m_fnsy.nextSync == -1) {
        if (m_fnsy.swnf == 0)
            info1(*this, evt_info, "syncword not found");
        else {
    ...
    if (m_fnsy.nextSync == 0) {
        if (m_fnsy.swnf) {
            info1(*this, evt_info, "syncword not found {} times", (long unsigned int)m_fnsy.swnf);
            m_fnsy.swnf = 0;
        } else {
            info1(*this, evt_info, "syncword found at pos 0");
            m_f_decode_ready = true;
        }
    }
    if (m_fnsy.nextSync > 0) { info1(*this, evt_info, "syncword found at pos {}", m_fnsy.nextSync); }
...
void Audio::setDecoderItems() {
    ...
    if (m_decoder->arg1()) info1(*this, evt_info, "{}", m_decoder->arg1());
    if (m_decoder->getAudioDataStart() > 0) { // only flac-ogg, native flac sets audioDataStart in readFlacHeader()
        ...
        info1(*this, evt_info, "AudioDataStart: {}", m_audioDataStart);
...
uint32_t Audio::decodeError(int8_t res, uint8_t* data, int32_t bytesDecoded) {
    ...
    if (m_codec == CODEC_AAC && res == -21) { // mono <-> stereo change
        ...
        if ((data[0] == 0xFF) || ((data[1] & 0xF0) == 0xF0)) {
            ...
            if (channel_config != m_sbyt.channels) {
                ...
                info1(*this, evt_info, "AAC channel config changed to {}", m_sbyt.channels);
            }
        }
        m_decoder->reset();
        m_decoder->init();
    ...
    if (m_codec == CODEC_MP3) {
        if (res == MP3Decoder::MP3_NEED_RESTART) {
            info1(*this, evt_info, "" ANSI_ESC_RED "Network error" ANSI_ESC_RESET "");
            connecttohost(m_lastHost.get());
            return 0;
        }
    }
...
int Audio::sendBytes(uint8_t* data, size_t len) {
    ...
    if ((bytesDecoded == 0) && (m_codec != CODEC_VORBIS && m_codec != CODEC_FLAC)) { // unlikely framesize, exept VORBIS decodes lastSegmentTable
        info1(*this, evt_info, "framesize is 0, start decoding again");
        m_f_playing = false; // seek for new syncword
    ...
    switch (m_codec) {
        ...
        case CODEC_AAC:
            ...
            if (!m_sbyt.isPS && m_decoder->val1()) { // only change 0 -> 1
                m_sbyt.isPS = 1;
                info1(*this, evt_info, "Parametric Stereo");
            } else
        ...
        case CODEC_FLAC:
            ...
            if (st) { info1(*this, evt_streamtitle, "{}", st); }
        ...
        case CODEC_OPUS:
            ...
            if (m_decoder->arg1()) {
                if (m_sbyt.opus_mode != m_decoder->arg1()) {
                    info1(*this, evt_info, m_decoder->arg1());
...
void Audio::calculateAudioTime(uint16_t bytesDecoderIn, uint16_t samples_decoder_out) {
        ...
        if (m_cat.nominalBitRate) {
            ...
            if ((abs(m_cat.avrBitRate - m_cat.oldAvrBitrate < 50)) && !m_cat.avrBitrateStable && m_cat.avrBitRate > 1000) {
                ...
                if (m_cat.brCounter > 6) {
                    ...
                    info1(*this, evt_bitrate, "{}", m_cat.avrBitrateStable);
                    info1(*this, evt_info, "estimated bitrate (b/s): {}", m_cat.avrBitrateStable);
                }
            }
    ...
    if (m_syltLines.size()) {
        ...
        if (m_audioCurrentTime * 1000 > m_syltTimeStamp[m_cat.syltIdx]) {
            info1(*this, evt_lyrics, "{}", m_syltLines[m_cat.syltIdx].c_get());
            m_cat.syltIdx++;
        }
    }
}
...
bool Audio::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t MCLK) {
    ...
    trim(audioI2SVers);
    info1(*this, evt_info, "audioI2S {}", audioI2SVers);
...
bool Audio::setTimeOffset(int sec) { // fast forward or rewind the current position in seconds
    // info1(*this, evt_info, "time offset {} sec", sec);
    if ((m_dataMode != AUDIO_LOCALFILE) && (m_streamType != ST_WEBFILE)) {
...
void Audio::setI2SCommFMT_LSB(bool commFMT) {
    ...
    if (commFMT) {
        info1(*this, evt_info, "commFMT = LSBJ (Least Significant Bit Justified)");
    } else {
        info1(*this, evt_info, "commFMT = Philips");
    }
...
bool Audio::readID3V1Tag() {
    ...
    auto readID3Field = [&](ps_ptr<char>& field, const uint8_t* src, size_t len, const char* label = nullptr) {
        ...
        if (field.strlen() > 0 && label) {
            ...
            info1(*this, evt_id3data, "{}", field.get());
        }
    };
    ...
    if (InBuff.bufferFilled() == 128 && startsWith((const char*)p, "TAG")) {
        ...
        info1(*this, evt_info, zeroByte ? "ID3 version: 1" : "ID3 Version 1.1");

        if (zeroByte == 0) {
            sprintf(chBuff.get(), "Track Number: %d", track);
            info1(*this, evt_id3data, "{}", chBuff.get());
        }
    ...
    if (InBuff.bufferFilled() == 227 && startsWith((const char*)p, "TAG+")) { // "TAG+" "can exist as an extension, does not overwrite" TAG"
        info1(*this, evt_info, "ID3 version: 1 - Enhanced TAG");
...
boolean Audio::streamDetection(uint32_t bytesAvail) {
    ...
    if (m_sdet.tmr_slow + 1000 < millis()) {
        ...
        if (m_sdet.cnt_slow > 100) info1(*this, evt_info, "slow stream, dropouts are possible");
        m_sdet.cnt_slow = 0;
    }
    ...
    if (m_sdet.tmr_lost < millis()) {
        ...
        if (m_sdet.cnt_lost == 5) { // 5s no data?
            m_sdet.cnt_lost = 0;
            info1(*this, evt_info, "Stream lost -> try new connection");
            m_f_reset_m3u8Codec = false;
            InBuff.reset();
            httpPrint(m_lastHost.get());
            return true;
...
uint8_t Audio::determineCodec(uint8_t presumed_codec) {
    ...
    if (presumed_codec == CODEC_AAC || presumed_codec == CODEC_MP3) { // only webstream, is AAC or MP3?
        ...
        info1(*this, evt_info, "contentType is {}, but %s found", codecname[presumed_codec], codecname[mimeType]);
        return mimeType;
    }
