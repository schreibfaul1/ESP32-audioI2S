...
#else
    switch (n) {
        case 3: ((uint8_t*)&tmp)[2] = ((uint8_t*)mem)[2];
        case 2: ((uint8_t*)&tmp)[1] = ((uint8_t*)mem)[1];
...
__unused static /*inline*/ uint32_t faad_getbits_rev(bitfile* ld, uint32_t n DEBUGDEC) {
    uint32_t ret;
    if (n == 0) return 0;
...
static uint8_t faad_check_CRC(bitfile* ld, uint16_t len) {
    ...
    #define GPOLY 0435
    faad_rewindbits(ld);
    CRC = (unsigned int)~faad_getbits(ld, 8 DEBUGVAR(1, 999, "faad_check_CRC(): CRC")) & 0xFF; /* CRC is stored inverted */
    bytes = len >> 3;
    rem = len & 0x7;
    for (; bytes > 0; bytes--) { r = crc_table_G8[(r ^ faad_getbits(ld, 8 DEBUGVAR(1, 998, ""))) & 0xFF]; }
    for (; rem > 0; rem--) { r = ((r << 1) ^ (((faad_get1bit(ld DEBUGVAR(1, 998, "")) & 1) ^ ((r >> 7) & 1)) * GPOLY)) & 0xFF; }
    if (r != CRC)
    //  if (0)
    {
        return 28;
...
static uint8_t tabFlipbits[256] = {0, 128, 64, 192, 32, 160, 96,  224, 16, 144, 80, 208, 48, 176, 112, 240, 8,  136, 72, 200, 40, 168, 104, 232, 24, 152, 88, 216, 56, 184, 120, 248,
