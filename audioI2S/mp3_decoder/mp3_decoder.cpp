...
int UnpackSideInfo(MP3DecInfo *mp3DecInfo, unsigned char *buf) {
    ...
    if (fh->ver == MPEG1) {
        ...
        for (ch = 0; ch < mp3DecInfo->nChans; ch++)
            for (bd = 0; bd < MAX_SCFBD; bd++) si->scfsi[ch][bd] = GetBits(bsi, 1);
    } else {
        /* MPEG 2, MPEG 2.5 */
...
void MP3GetLastFrameInfo(HMP3Decoder hMP3Decoder, MP3FrameInfo *mp3FrameInfo) {
    ...
    if (!mp3DecInfo || mp3DecInfo->layer != 3) {
        ...
        mp3FrameInfo->outputSamps =
                mp3DecInfo->nChans
                        * (int) samplesPerFrameTab[mp3DecInfo->version][mp3DecInfo->layer
                                - 1];
...
int MP3Decode(HMP3Decoder hMP3Decoder, unsigned char **inbuf, int *bytesLeft, short *outbuf, int useSize){
    ...
    *bytesLeft -= (fhBytes + siBytes);
    ...
    if (mp3DecInfo->bitrate == 0 || mp3DecInfo->freeBitrateFlag) {
        ...
        mp3DecInfo->nSlots = mp3DecInfo->freeBitrateSlots
    ...
    for (gr = 0; gr < mp3DecInfo->nGrans; gr++) {
        for (ch = 0; ch < mp3DecInfo->nChans; ch++) {
            ...
            offset = UnpackScaleFactors(mp3DecInfo, mainPtr, &bitOffset,
                    mainBits, gr, ch);
            ...
            prevBitOffset = bitOffset;
            offset = DecodeHuffman(mp3DecInfo, mainPtr, &bitOffset,
                    huffBlockBits, gr, ch);
            if (offset < 0) {
                MP3ClearBadFrame(mp3DecInfo, outbuf);
        ...
        if (Subband(mp3DecInfo,
                outbuf + gr * mp3DecInfo->nGranSamps * mp3DecInfo->nChans)
                < 0) {
            MP3ClearBadFrame(mp3DecInfo, outbuf);
...
static int DecodeHuffmanPairs(int *xy, int nVals, int tabIdx, int bitsLeft,
    ...
    tabType = (HuffTabType)huffTabLookup[tabIdx].tabType;

    ASSERT(!(nVals & 0x01)); ASSERT(tabIdx < HUFF_PAIRTABS); ASSERT(tabIdx >= 0); ASSERT(tabType != invalidTab);
...
/* __attribute__ ((section (".data"))) */int DecodeHuffman(
        MP3DecInfo *mp3DecInfo, unsigned char *buf, int *bitOffset,
    ...
    for (i = 0; i < 3; i++) {
        bitsUsed = DecodeHuffmanPairs(hi->huffDecBuf[ch] + rEnd[i],
    ...
    hi->nonZeroBound[ch] += DecodeHuffmanQuads(hi->huffDecBuf[ch] + rEnd[3],
            MAX_NSAMP - rEnd[3], sis->count1TableSelect, bitsLeft, buf,
            *bitOffset);
...
void IntensityProcMPEG1(int x[MAX_NCHAN][MAX_NSAMP], int nSamps,
        FrameHeader *fh, ScaleFactorInfoSub *sfis, CriticalBandInfo *cbi,
...
void IntensityProcMPEG2(int x[MAX_NCHAN][MAX_NSAMP], int nSamps,
        FrameHeader *fh, ScaleFactorInfoSub *sfis, CriticalBandInfo *cbi,
        ScaleFactorJS *sfjs, int midSideFlag, int mixFlag, int mOut[2]) {
    int i, j, k, n, r, cb, w;
...
/* __attribute__ ((section (".data"))) */static void AntiAlias(int *x,
        int nBfly) {
    int k, a0, b0, c0, c1;
    const int *c;
 ...
 **************************************************************************************/
/*__attribute__ ((section (".data"))) */static void WinPrevious(int *xPrev,
        int *xPrevWin, int btPrev) {
 ...
 **************************************************************************************/
/*__attribute__ ((section (".data")))*/static int FreqInvertRescale(int *y,
        int *xPrev, int blockIdx, int es) {
    int i, d, mOut;
...
const int fastWin36[18] PROGMEM = { 0x42aace8b, 0xc2e92724, 0x47311c28,
        0xc95f619a, 0x4a868feb, 0xd0859d8c, 0x4c913b51, 0xd8243ea0, 0x4d413ccc,
        0xe0000000, 0x4c913b51, 0xe7dbc161, 0x4a868feb, 0xef7a6275, 0x47311c28,
        0xf6a09e67, 0x42aace8b, 0xfd16d8dd, };
...
/*__attribute__ ((section (".data")))*/static int IMDCT36(int *xCurr,
        int *xPrev, int *y, int btCurr, int btPrev, int blockIdx, int gb) {
 ...
 **************************************************************************************/
// barely faster in RAM
/*__attribute__ ((section (".data")))*/static int IMDCT12x3(int *xCurr,
        int *xPrev, int *y, int btPrev, int blockIdx, int gb) {
 ...
 **************************************************************************************/
/* __attribute__ ((section (".data"))) */static int HybridTransform(int *xCurr,
        int *xPrev, int y[BLOCK_SIZE][NBANDS], SideInfoSub *sis, BlockCount *bc) {
    int xPrevWin[18], currWinIdx, prevWinIdx;
    int i, j, nBlocksOut, nonZero, mOut;
    int fiBit, xp;

    ASSERT(bc->nBlocksLong <= NBANDS); ASSERT(bc->nBlocksTotal <= NBANDS); ASSERT(bc->nBlocksPrev <= NBANDS);
 ...
 **************************************************************************************/
// a bit faster in RAM
/*__attribute__ ((section (".data")))*/int IMDCT(MP3DecInfo *mp3DecInfo, int gr,
        int ch) {
 ...
 * Return:      0 on success,  -1 if null input pointers
 **************************************************************************************/
/*__attribute__ ((section (".data"))) */int Subband(MP3DecInfo *mp3DecInfo,
        short *pcmBuf) {
    int b;
    //HuffmanInfo *hi;
    IMDCTInfo *mi;
    ...
    if (mp3DecInfo->nChans == 2) {
        ...
        for (b = 0; b < BLOCK_SIZE; b++) {
            FDCT32(mi->outBuf[0][b], sbi->vbuf + 0 * 32, sbi->vindex,
                    (b & 0x01), mi->gb[0]);
            FDCT32(mi->outBuf[1][b], sbi->vbuf + 1 * 32, sbi->vindex,
                    (b & 0x01), mi->gb[1]);
...
void PolyphaseMono(short *pcm, int *vbuf, const int *coefBase)
{
    ...
    rndVal = (uint64_t)( 1 << (DEF_NFRACBITS - 1 + (32 - CSHIFT)) );
